...
- `bullpen.py` — **what a team still has LEFT in the pen**, graded. The Arm Ledger says where a game has been; this says whether the lead keeps growing, which is the actual bet. Pulls the active roster and every arm's season line (two requests per team, **cached for the day**, so it never rides the poll loop and cannot affect golden detection). Grades each arm against the screen Nick brought in — K-BB%, K%, BB%, ERA — and rolls up a pen verdict (`INTACT`/`MIXED`/`SOFT`/`NEARLY OUT`/`EMPTY`). Two rules keep it honest: arms under `MIN_BATTERS_FACED` (30) return `THIN` rather than a verdict, because a 0.00 ERA over six batters would otherwise grade `ATTACK`; and starters (`gamesStarted/gamesPitched >= 0.5`) are split out of the pen count entirely, since they are not what a trailing team reaches for next. **Stuff+, xFIP/SIERA, Barrel% and SwStr% are not available from this API and are deliberately not approximated** — a fabricated Stuff+ would be worse than none. Returning `None` means "roster unavailable", never "nobody left".
